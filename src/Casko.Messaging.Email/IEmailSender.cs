...
public interface IEmailSender
{
    ...
    Task SendAsync(EmailDelivery delivery, CancellationToken cancellationToken = default);

    /// <summary>Sends multiple independent email messages.</summary>
    /// <param name="deliveries">The deliveries to send.</param>
    /// <param name="cancellationToken">A token that can cancel the operation.</param>
    Task SendAsync(IEnumerable<EmailDelivery> deliveries, CancellationToken cancellationToken = default);
}
